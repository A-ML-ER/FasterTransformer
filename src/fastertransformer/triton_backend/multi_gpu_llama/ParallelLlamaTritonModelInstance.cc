...
{
    ParallelLlamaTritonModelInstance<T>* model  = reinterpret_cast<ParallelLlamaTritonModelInstance<T>*>(ctx);
    auto                               result = ParallelLlamaTritonModelInstance<T>::convert_outputs(*output_tensors);

    model->stream_cb_(result, model->stream_ctx_);
}

template<typename T>
ParallelLlamaTritonModelInstance<T>::ParallelLlamaTritonModelInstance(
    std::unique_ptr<ft::ParallelLlama<T>>                     llama,
    std::shared_ptr<ft::ParallelLlamaWeight<T>>               llama_weight,
...
ParallelLlamaTritonModelInstance<T>::forward(std::shared_ptr<std::vector<triton::Tensor>> input_tensors)
{
    ft::FT_CHECK(false);
    return nullptr;
}
...
template<typename T>
std::unordered_map<std::string, ft::Tensor> ParallelLlamaTritonModelInstance<T>::convert_inputs(
    std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
    ...
    for (int i = 0; i < request_batch_size; ++i) {
        h_total_output_lengths_[i] = reinterpret_cast<const uint32_t*>(input_tensors->at("request_output_len").data)[i]
                                     + input_data_len + (continue_interactive ? llama_->getStep() : 0);
    }
...
ParallelLlamaTritonModelInstance<T>::convert_outputs(const std::unordered_map<std::string, ft::Tensor>& output_tensors)
{
...
template<typename T>
std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> ParallelLlamaTritonModelInstance<T>::forward(
    std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
    ...
    if (input_tensors->count("is_return_context_embeddings")
        ...
        output_tensors.insert({"context_embeddings",
                               ft::Tensor{ft::MEMORY_GPU,
                                          ft::TYPE_FP32,
                                          std::vector<size_t>{request_batch_size, beam_width, llama_->getHiddenUnits()},
                                          d_output_ctx_emb_}});
    }

    try {
        if (stream_cb_ != nullptr) {
            llama_->registerCallback(triton_stream_callback<T>, this);
        }

        llama_->forward(&output_tensors, &ft_input_tensors, llama_weight_.get());

        if (stream_cb_ != nullptr) {
            llama_->unRegisterCallback();
        }
    }
    catch (...) {
...
template<typename T>
void ParallelLlamaTritonModelInstance<T>::allocateBuffer(const size_t request_batch_size,
                                                       ...
                                                       const size_t request_output_len)
{
    ...
    d_output_ctx_emb_   = (float*)(allocator_->reMalloc(
        d_output_ctx_emb_, sizeof(float) * request_batch_size * beam_width * llama_->getHiddenUnits(), false));
    d_cum_log_probs_ =
...
template<typename T>
void ParallelLlamaTritonModelInstance<T>::freeBuffer()
{
...
template struct ParallelLlamaTritonModelInstance<float>;
template struct ParallelLlamaTritonModelInstance<half>;
#ifdef ENABLE_BF16
template struct ParallelLlamaTritonModelInstance<__nv_bfloat16>;
#endif
