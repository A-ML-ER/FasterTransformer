...
template<typename T>
LlamaTritonModel<T>::LlamaTritonModel(size_t                                     max_seq_len,
                                    size_t                                     head_num,
                                        ...
                                        std::shared_ptr<ft::AbstractCustomComm> custom_all_reduce_comm)
{
    ...
    auto llama =
        std::make_unique<ft::Llama<T>>(ft::Llama<T>(0,  // max_batch_size, FT will adjust the buffer automatically.
    ...
    return std::unique_ptr<LlamaTritonModelInstance<T>>(new LlamaTritonModelInstance<T>(std::move(llama),
