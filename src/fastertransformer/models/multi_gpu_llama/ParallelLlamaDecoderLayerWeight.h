...
template<typename T>
struct ParallelLlamaDecoderLayerWeight {
public:
    ParallelLlamaDecoderLayerWeight() = default;
    ParallelLlamaDecoderLayerWeight(const int int8_mode);
    ParallelLlamaDecoderLayerWeight(const int        hidden_units,
                                  const int        inter_size,
                                  const int        tensor_para_size,
                                  const int        tensor_para_rank,
                                  const int        int8_mode          = 0,
                                  llamaVariantParams llama_variant_params = {});
    ~ParallelLlamaDecoderLayerWeight();
    ParallelLlamaDecoderLayerWeight(const ParallelLlamaDecoderLayerWeight& other);
    ParallelLlamaDecoderLayerWeight& operator=(const ParallelLlamaDecoderLayerWeight& other);
...
private:
    void copyFrom(const ParallelLlamaDecoderLayerWeight& other);
    void setWeightPtr();
    void mallocWeights();
...
protected:
    ...
    llamaVariantParams llama_variant_params_;
