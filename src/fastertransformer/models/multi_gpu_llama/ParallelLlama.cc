...
#include "src/fastertransformer/models/multi_gpu_llama/ParallelLlama.h"
...
template<typename T>
void ParallelLlama<T>::initialize()
{
    llama_context_decoder_ = new ParallelLlamaContextDecoder<T>(0,
                                                            0,
                                                            head_num_,
                                                            size_per_head_,
                                                            inter_size_,
    ...
    llama_decoder_ = new ParallelLlamaDecoder<T>(0,
                                             head_num_,
                                             size_per_head_,
                                             inter_size_,
                                             num_layer_,
...
template<typename T>
void ParallelLlama<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void ParallelLlama<T>::allocateBuffer(size_t batch_size,
                                    size_t beam_width,
                                    ...
                                    bool   is_return_context_cum_log_probs)
{
    ...
    if (llama_variant_params_.use_attention_linear_bias) {
...
template<typename T>
void ParallelLlama<T>::freeBuffer()
{
...
template<typename T>
ParallelLlama<T>::ParallelLlama(size_t                              max_batch_size,
                            size_t                              max_seq_len,
                            size_t                              max_input_len,
                            size_t                              beam_width,
                            size_t                              head_num,
    ...
    layernorm_eps_(llama_variant_params.layernorm_eps),
...
template<typename T>
ParallelLlama<T>::ParallelLlama(ParallelLlama<T> const& llama):
    BaseLayer(llama),
    head_num_(llama.head_num_),
    size_per_head_(llama.size_per_head_),
    inter_size_(llama.inter_size_),
    num_layer_(llama.num_layer_),
    expert_num_(llama.expert_num_),
    moe_k_(llama.moe_k_),
    moe_layer_index_(llama.moe_layer_index_),
    ...
    beam_search_diversity_rate_(llama.beam_search_diversity_rate_),
    layernorm_eps_(llama.llama_variant_params_.layernorm_eps),
    llama_variant_params_(llama.llama_variant_params_),
    hidden_units_(llama.hidden_units_),
    top_k_(llama.top_k_),
    top_p_(llama.top_p_),
    random_seed_(llama.random_seed_),
    temperature_(llama.temperature_),
    len_penalty_(llama.len_penalty_),
    repetition_penalty_(llama.repetition_penalty_),
...
template<typename T>
void ParallelLlama<T>::computeContextCumLogProbs(float*                      cum_log_probs,
                                               ...
                                               const ParallelLlamaWeight<T>* llama_weights)
{
...
template<typename T>
void ParallelLlama<T>::registerCallback(callback_sig* fn, void* ctx)
{
...
template<typename T>
void ParallelLlama<T>::unRegisterCallback()
{
...
template<typename T>
void ParallelLlama<T>::forward(std::vector<Tensor>*        output_tensors,
                             ...
                             const ParallelLlamaWeight<T>* llama_weights)
{
    ...
    forward(&output_tensors_map, &input_tensors_map, llama_weights);
}

template<typename T>
void ParallelLlama<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
                             ...
                             const ParallelLlamaWeight<T>*                    llama_weights)
{
    ...
    if (llama_variant_params_.has_positional_encoding && session_len_ > llama_weights->getMaxSeqLen()) {
        FT_LOG_ERROR("The session_len_ (%d) of request is longer than max_seq_len (%d) of embedding table."
                     ...
                     session_len_,
                     llama_weights->getMaxSeqLen(),
                     llama_weights->getMaxSeqLen());
        session_len_ = llama_weights->getMaxSeqLen();
    }
    ...
    if (continue_gen) {
        ...
        invokeTileLlamaInputs(tiled_input_ids_buf_,
        ...
        if (use_loaded_p_prompt_embedding) {
            for (int bs_id = 0; bs_id < batch_size; ++bs_id) {
                ...
                bool                     valid_task_name_id   = task_id < llama_weights->prompt_learning_table.size();
                if (valid_task_name_id) {
                    p_prompt_tuning_pair = llama_weights->prompt_learning_table.at(task_id);
                }
                else {
        ...
        if (has_p_prompt_tuning_ || has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
            PUSH_RANGE("input tiling and init");
            invokeTilePromptInputs(tiled_input_ids_buf_,
                                      tiled_input_lengths_buf_,
            ...
            if (llama_variant_params_.has_pre_decoder_layernorm) {
                ...
                invokeGeneralLayerNorm(context_decoder_normed_input_buf_,
                                       ...
                                       llama_weights->pre_decoder_layernorm.gamma,
                                       llama_weights->pre_decoder_layernorm.beta,
                                       layernorm_eps_,
            ...
            TensorMap decoder_input_tensors(
                {{"decoder_input",
                  Tensor(MEMORY_GPU,
                         ...
                         llama_variant_params_.has_pre_decoder_layernorm ? context_decoder_normed_input_buf_ :
                                                                         context_decoder_input_buf_)},
                 {"attention_mask",
                  Tensor(MEMORY_GPU,
                         data_type,
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
                ...
                std::unordered_map<std::string, Tensor> decoder_input_tensors(
                    {{"decoder_input",
                      Tensor(MEMORY_GPU,
                             ...
                             llama_variant_params_.has_pre_decoder_layernorm ?
            ...
            if (!fill_caches_only && pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
                ...
                T* decoder_output_final_buf =
                    llama_variant_params_.has_post_decoder_layernorm ? normed_decoder_output_buf_ : decoder_output_buf_;
...
template<typename T>
void ParallelLlama<T>::sendTensorsToFirstPipelineNode(std::unordered_map<std::string, Tensor>*       output_tensors,
                                                    const std::unordered_map<std::string, Tensor>* input_tensors)
{
...
template<typename T>
void ParallelLlama<T>::setOutputTensors(std::unordered_map<std::string, Tensor>*       output_tensors,
                                      const std::unordered_map<std::string, Tensor>* input_tensors,
...
template<typename T>
size_t ParallelLlama<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t ParallelLlama<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t ParallelLlama<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t ParallelLlama<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
size_t ParallelLlama<T>::getHiddenUnits()
{
    return hidden_units_;
}

template<typename T>
bool* ParallelLlama<T>::getFinishBuffer()
{
    return finished_buf_;
}

template<typename T>
size_t ParallelLlama<T>::getStep()
{
    return step_;
}

template class ParallelLlama<float>;
template class ParallelLlama<half>;
#ifdef ENABLE_BF16
template class ParallelLlama<__nv_bfloat16>;
#endif
