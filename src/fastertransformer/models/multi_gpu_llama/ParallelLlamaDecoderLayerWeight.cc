...
template<typename T>
ParallelLlamaDecoderLayerWeight<T>::ParallelLlamaDecoderLayerWeight(const int        hidden_units,
...
template<typename T>
ParallelLlamaDecoderLayerWeight<T>::ParallelLlamaDecoderLayerWeight(const int int8_mode): int8_mode_(int8_mode)
{
}

template<typename T>
ParallelLlamaDecoderLayerWeight<T>::~ParallelLlamaDecoderLayerWeight()
{
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::copyFrom(const ParallelLlamaDecoderLayerWeight& other)
{
    ...
    if (llama_variant_params_.has_adapters) {
        // Copy adapter biases regardless of int8 mode
        cudaD2Dcpy(weights_ptr[13], other.weights_ptr[13], llama_variant_params_.adapter_inter_size / tensor_para_size_);
        cudaD2Dcpy(weights_ptr[15], other.weights_ptr[15], hidden_units_);
        cudaD2Dcpy(weights_ptr[17], other.weights_ptr[17], llama_variant_params_.adapter_inter_size / tensor_para_size_);
    ...
    if (int8_mode_ == 0) {
        ...
        if (llama_variant_params_.has_adapters) {
            cudaD2Dcpy(weights_ptr[12],
                       ...
                       hidden_units_ * llama_variant_params_.adapter_inter_size / tensor_para_size_);
            cudaD2Dcpy(weights_ptr[14],
                       other.weights_ptr[14],
                       llama_variant_params_.adapter_inter_size / tensor_para_size_ * hidden_units_);
        ...
        if (int8_mode_ == 1) {
            ...
            if (llama_variant_params_.has_adapters) {
                cudaD2Dcpy(weight_only_scale_ptr[4],
                           ...
                           llama_variant_params_.adapter_inter_size / tensor_para_size_);
...
template<typename T>
ParallelLlamaDecoderLayerWeight<T>::ParallelLlamaDecoderLayerWeight(const ParallelLlamaDecoderLayerWeight& other):
...
template<typename T>
ParallelLlamaDecoderLayerWeight<T>&
ParallelLlamaDecoderLayerWeight<T>::operator=(const ParallelLlamaDecoderLayerWeight& other)
{
    hidden_units_       = other.hidden_units_;
    inter_size_         = other.inter_size_;
    tensor_para_size_   = other.tensor_para_size_;
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::setWeightPtr()
{
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::mallocWeights()
{
    ...
    if (llama_variant_params_.has_adapters) {
        deviceMalloc(&weights_ptr[13], llama_variant_params_.adapter_inter_size / tensor_para_size_);
        deviceMalloc(&weights_ptr[15], hidden_units_);
        deviceMalloc(&weights_ptr[17], llama_variant_params_.adapter_inter_size / tensor_para_size_);
    ...
    if (int8_mode_ == 0) {
        ...
        if (llama_variant_params_.has_adapters) {
            deviceMalloc(&weights_ptr[12], hidden_units_ * llama_variant_params_.adapter_inter_size / tensor_para_size_);
            deviceMalloc(&weights_ptr[14], llama_variant_params_.adapter_inter_size / tensor_para_size_ * hidden_units_);
            deviceMalloc(&weights_ptr[16], hidden_units_ * llama_variant_params_.adapter_inter_size / tensor_para_size_);
            deviceMalloc(&weights_ptr[18], llama_variant_params_.adapter_inter_size / tensor_para_size_ * hidden_units_);
        }
    }
    else {
        ...
        if (int8_mode_ == 1) {
            ...
            if (llama_variant_params_.has_adapters) {
                ...
                deviceMalloc(&weight_only_scale_ptr[4], llama_variant_params_.adapter_inter_size / tensor_para_size_);
                deviceMalloc(&weight_only_scale_ptr[5], hidden_units_);
                deviceMalloc(&weight_only_scale_ptr[6], llama_variant_params_.adapter_inter_size / tensor_para_size_);
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper, int hidden_dim)
{
    ...
    size_t real_num_sparse_weights = llama_variant_params_.has_adapters ? num_sparse_weights : (num_sparse_weights - 4);
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::transposeWeight()
{
...
template<typename T>
void ParallelLlamaDecoderLayerWeight<T>::transposeCalibrateQuantizeWeight()
{
...
template struct ParallelLlamaDecoderLayerWeight<float>;
template struct ParallelLlamaDecoderLayerWeight<half>;
#ifdef ENABLE_BF16
template struct ParallelLlamaDecoderLayerWeight<__nv_bfloat16>;
#endif
