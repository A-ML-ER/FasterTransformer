...
#include "src/fastertransformer/models/multi_gpu_llama/ParallelLlamaContextDecoder.h"
...
template<typename T>
void ParallelLlamaContextDecoder<T>::initialize()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    self_attention_layer_ = new TensorParallelLlamaContextAttentionLayer<T>(max_batch_size_,
                                                                          max_seq_len_,
                                                                          head_num_,
                                                                          size_per_head_,
                                                                          tensor_para_,
...
template<typename T>
void ParallelLlamaContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void ParallelLlamaContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void ParallelLlamaContextDecoder<T>::freeBuffer()
{
...
template<typename T>
bool ParallelLlamaContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool ParallelLlamaContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool ParallelLlamaContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int ParallelLlamaContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
ParallelLlamaContextDecoder<T>::ParallelLlamaContextDecoder(size_t               max_batch_size,
                                                        size_t               max_seq_len,
                                                        size_t               head_num,
...
template<typename T>
ParallelLlamaContextDecoder<T>::ParallelLlamaContextDecoder(ParallelLlamaContextDecoder<T> const& decoder):
...
template<typename T>
ParallelLlamaContextDecoder<T>::~ParallelLlamaContextDecoder()
{
...
template<typename T>
void ParallelLlamaContextDecoder<T>::forward(
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            if (has_adapters_) {
                ...
                ffn_layer_->forward(&ffn_output_tensors,
                                    &ffn_input_tensors,
                                    &llama_decoder_layer_weight->at(l)->after_attention_adapter_weights);
            }
...
template class ParallelLlamaContextDecoder<float>;
template class ParallelLlamaContextDecoder<half>;
#ifdef ENABLE_BF16
template class ParallelLlamaContextDecoder<__nv_bfloat16>;
#endif
